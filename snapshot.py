...
def obtain_balances():
	...
	while True:
		data = {"jsonrpc": "2.0", "params": ["asset", "get_asset_holders", [info['asset_hold_id'], n_from, 100]], "method": "call",
			"id": n_call}
		try:
			rtn = requests.post(HTTP_NODE, data=json.dumps(data))
...
def obtain_accounts():
	next_account = ""
	cnt = 0
	Redisdb.delete('accounts')
	while True:
		batch = []
		accounts = Bitshares.rpc.lookup_accounts(next_account, 100)
		for a in accounts:
			Redisdb.rpush('accounts', a[0]+"#"+a[1])
			cnt += 1
		next_account = a[0]+" "
		Redisdb.rpush("messages", datetime.datetime.now().isoformat() + " count:"+str(cnt)+" Account "+a[1])
		if len(accounts) < 100:
			break
...
def do_snapshot(dat):
	...
	obtain_balances()
	if snap_info['asset_hold_minimum'] == 0:
		obtain_accounts()
